...
output_file = "../asr_new_suprused.jsonl"
...
for i, text in enumerate(test_sentences):
    references[f"test_{i+1:02d}.wav"] = text
...
print(f"\nProcessing audio files from: {audio_dir}\n")
...
with open(output_file, "w") as f:
    for audio_path in sorted(glob.glob(os.path.join(audio_dir, "test_*.wav"))):
        ...
        if filename not in references:
            print(f"Skipping {filename} - no reference found")
            continue
        ...
        if hasattr(transcriptions[0], "text"):
            hypothesis = transcriptions[0].text
        else:
            hypothesis = transcriptions[0]

        ref_text = references[filename].lower().strip()
        ...
        print(f"{filename} | WER: {sample_wer:.4f} | CER: {sample_cer:.4f}")
