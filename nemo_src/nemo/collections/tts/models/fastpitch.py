...
from nemo.collections.tts.losses.fastpitchloss import DurationLoss, EnergyLoss, MelLoss, PitchLoss, EmotionLoss
...
class FastPitchModel(SpectrogramGenerator, Exportable, FastPitchAdapterModelMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        emotion_loss_scale = cfg.get("emotion_loss_scale", default_prosody_loss_scale)
        ...
        self.emotion_loss_fn = EmotionLoss(loss_scale=emotion_loss_scale)
        ...
        emotion_predictor = instantiate(self._cfg.get("emotion_predictor", None))
        emotion_embedding_kernel_size = cfg.get("emotion_embedding_kernel_size", 0)
    ...
    @typecheck(
        input_types={
            ...
            "emotion": NeuralType(('B', 'T_audio'), RegressionValuesType(), optional=True),
    ...
    def training_step(self, batch, batch_idx):
        attn_prior, durs, speaker, energy, emotion, reference_audio, reference_audio_len = (
            None,
            None,
            None,
            None,
        ...
        if self.learn_alignment:
            ...
            emotion = batch_dict.get("emotion", None)
        ...
        emotion_loss = self.emotion_loss_fn(emotion_predicted = emotion_pred, emotion_tgt=emotion_tgt, length=text_lens)
        loss += pitch_loss + energy_loss
        if emotion_tgt is not None:
            loss += emotion_loss
        ...
        if emotion_tgt is not None:
            self.log("t_emotion_loss", emotion_loss)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        print(f"[VAL DEBUG] mel={mel_loss.item():.4f} dur={dur_loss.item():.4f} "
              f"pitch={pitch_loss.item():.4f} energy={energy_loss} "
              f"emotion={emotion_loss.item():.4f} "
              f"emotion_tgt_none={emotion_tgt is None}")
        ...
        val_outputs = {
            ...
            "emotion_loss": emotion_loss if emotion_tgt is not None else None,
    ...
    def on_validation_epoch_end(self):
        ...
        print(f"[EPOCH END] val_loss={val_loss} mel={mel_loss} dur={dur_loss} pitch={pitch_loss}")
        print(f"[EPOCH END] n_outputs={len(self.validation_step_outputs)}")
        ...
        if self.validation_step_outputs[0]["emotion_loss"] is not None:
            emotion_loss = collect("emotion_loss")
            self.log("val_emotion_loss", emotion_loss, sync_dist=True)

        spec_target = self.validation_step_outputs[0]["mel_target"]
        spec_predict = self.validation_step_outputs[0]["mel_pred"]
