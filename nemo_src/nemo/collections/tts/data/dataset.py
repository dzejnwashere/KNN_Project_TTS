...
# we have added emotion embedding and emotions
...
class TTSDataset(Dataset):
    ...
    def add_emotion(self, **kwargs):
        emotion_folder = Path(self.sup_data_path) / "emotion"
        if not emotion_folder.exists():
            raise FileNotFoundError(
                f"Emotion folder not found: {emotion_folder}. "
                "Please run extract_emotion_data.py first."
            )
    ...
    def __getitem__(self, index):
        ...
        # Load emotion if needed
        emotion, emotion_length = None, None
        if Emotion in self.sup_data_types_set:
            emotion_path = Path(self.sup_data_path) / Emotion.name / f"{rel_audio_path_as_text_id}.pt"
            if emotion_path.exists():
                emotion = torch.load(emotion_path, weights_only=True).float()
            else:
                raise FileNotFoundError(
                    f"Emotion file not found: {emotion_path}. "
                    "Run emotion extraction script first."
                )
            emotion_length = torch.tensor(emotion.shape[0]).long()
        else:
            emotion = torch.tensor([])
            emotion_length = torch.tensor(0).long()
    ...
    def general_collate_fn(self, batch):
        ...
        max_emotions_len = max([e.shape[0] for e in emotions]) if len(emotions) > 0 else None
        ...
        for i, sample_tuple in enumerate(batch):
            ...
            if Emotion in self.sup_data_types_set:
                emotions.append(general_padding(emotion, emotion_length.item(), max_emotions_len))
        ...
        data_dict = {
            ...
            "emotion": torch.stack(emotions) if Emotion in self.sup_data_types_set else None,
            "emotion_lens": torch.stack(emotions_lengths) if Emotion in self.sup_data_types_set else None,
